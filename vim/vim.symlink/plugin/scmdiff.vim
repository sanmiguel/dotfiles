...
function! s:scmDiff(...)
    ...
    if g:scmDiffUseAbsPaths
        ...
        let cmd = g:scmDiffCommand . ' diff -b ' . g:scmDiffRev . ' ' . bufname('%') . ' > ' . tmpdiff
    endif
    echon cmd
    let cmdOutput = system(cmd)

    if v:shell_error && cmdOutput != ''
        echohl WarningMsg | echon g:scmDiffCommand . ' failed ' | echon cmdOutput
        return
    endif

    echo 'tmpfile: ' . b:scmDiffTmpfile
    let cmd = 'patch -R -p0 ' . b:scmDiffTmpfile . ' ' . tmpdiff
    let cmdOutput = system(cmd)

    if v:shell_error && cmdOutput != ''
        echohl WarningMsg | echon 'patch failed:' | echon cmdOutput
        return
    endif
